...
### Quick Setup
 ...
 - **getAveragePrice**: if set to TRUE, it will get ALL trading data for the trading pair for the last X minutes and find an average price for the coin. This avoids you buying pre-pumped coins. WARNING: It must download a lot of data (it took 5-7 seconds to download BNBBTC's trading data) so I would not recommend it unless you have good, fast Internet connection. Default value is set to FALSE.
 - **minutesAveragePrice**: sets how many minutes to download the data to calculate the average price for the coin.
 - **openBinanceWeb**: if set to TRUE it will open the Binance website with the trading pair.
